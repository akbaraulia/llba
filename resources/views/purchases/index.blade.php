                            ...
                            <td>
                                @if ($purchase->points_used > 0)
                                    ...
                                    <div class="text-success fw-bold">Rp
                                        {{ number_format($purchase->total_after_discount, 0, ',', '.') }}</div>
                                @else
                            ...
                            <td>
                                <div class="d-flex gap-1">
                                    <button class="btn btn-sm btn-outline-primary" data-bs-toggle="modal"
                                        data-bs-target="#detailModal{{ $purchase->id }}">Lihat</button>
                                    <a href="{{ route('purchases.receipt', $purchase) }}"
                                        class="btn btn-sm btn-outline-dark">Struk</a>
                                </div>
                            </td>
                        </tr>
                    @empty
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="row mt-2">
                                <div class="col-md-6">
                                    <p class="mb-1">Status:
                                        <strong>{{ $purchase->is_member ? 'Member' : 'Bukan Member' }}</strong></p>
                                    ...
                                    <p class="mb-1">Diskon Poin: <strong>Rp
                                            {{ number_format($purchase->points_discount_amount, 0, ',', '.') }}</strong>
                                    </p>
                                    <p class="mb-0">Max Poin Dipakai:
                                        <strong>{{ number_format($purchase->max_redeem_percentage, 2, ',', '.') }}%</strong>
                                    </p>
                                </div>
                                <div class="col-md-6 text-md-end">
                                    <p class="mb-1">Total Awal: <strong>Rp
                                            {{ number_format($purchase->total_before_discount, 0, ',', '.') }}</strong></p>
                                    <p class="mb-1">Total Bayar: <strong>Rp
                                            {{ number_format($purchase->total_after_discount, 0, ',', '.') }}</strong></p>
                                    <p class="mb-1">Dibayar: <strong>Rp
                                            {{ number_format($purchase->cash_paid, 0, ',', '.') }}</strong></p>
                                    <p class="mb-0">Kembalian: <strong>Rp
                                            {{ number_format($purchase->change_amount, 0, ',', '.') }}</strong></p>
                                </div>
                            </div>
                        </div>
                    </div>
