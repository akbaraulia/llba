...
<div class="panel-card p-3 p-lg-4">
    <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3">
        <form class="d-flex gap-2" method="GET" action="{{ route('users.index') }}">
            <input class="form-control" type="text" name="q" value="{{ $search }}" placeholder="Cari nama/email/telepon...">
                    ...
                    <td>
                        @if(auth()->user()?->isAdmin())
                            <div class="d-flex flex-wrap gap-1">
                                ...
                                <button class="btn btn-sm btn-outline-danger" data-bs-toggle="modal" data-bs-target="#deleteUserModal{{ $user->id }}">Hapus</button>
                            </div>
                        @else
